...
if($_POST){
    ...
    $dept=$_POST['dept'];
    $batch=$_POST['batch'];
    if(!empty($rollno)&&!empty($sname)&&!empty($gender)&&
            !empty($address)&&!empty($dob)&&!empty($sem)&&
            !empty($semail)&&!empty($dept)&&!empty($batch)){
        

//Inserting data in the database
        $link=mysqli_connect('localhost','root','','AIKTC');
        if(!$link){
            echo '<br>Unable to connect to Database.'
            .mysqli_connect_error();
        }
        $query="Insert into Students values ('$rollno','$sname','$gender',"
                . "'$address','$dob',"
